...
case class OutputLine(moduleOrg: String, moduleName: String, revision: String, artifact: String, ext: String, sha1: String, md5: String, depModuleOrg: String, depModuleName: String, depRevision: String, depArtifact: String, statusCode: String) {
  def format = {
    ...
    moduleOrg + ivyCoordSep + moduleName + ivyCoordSep + revision + sep + artifact + sep + ext + sep + sha1 + sep + md5 + sep + depIvy + sep + depArtifact + sep + statusCode + sep
  }
}
...
object OutputLine {
  def parse(s: String) = {
    ...
    s match {
      case FormatExpr(ivyCoords, artifact, ext, sha1, md5, depIvy, depArtifact, statusCode) => {
        ...
        OutputLine(parseIvyCoords(ivyCoords)._1, parseIvyCoords(ivyCoords)._2, parseIvyCoords(ivyCoords)._3, artifact, ext, sha1, md5, "", "", "", depIvy, statusCode)
      }
